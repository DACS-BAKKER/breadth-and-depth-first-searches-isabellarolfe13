/*
 * Isabella Rolfe
 * Sudoku using Depth First Search
 * 02/15/20
 */
import java.io.File;
...
public class Soduku {
	...
	//created a stack called StackSod
	public StackSod stack; 
	private Scanner BoardScanner;
	public File boardFile=new File("boardFileHard.txt");

	public Soduku() throws FileNotFoundException {
		for(int x=0;x<row;x++) {
			for(int y=0;y<col;y++) {
				board[x][y]=0;
			}
	...
	//main solving method
	public void solve() {
		while(!isBoardFilled(stack.Peek().getBoard())) {
			System.out.println("Stack size: " + stack.size());
			NodeSod myBoard=stack.Pop();
			...
			System.out.println("List of options" + option);
			System.out.println("------------");
			if(option!=null) {
